...
cor(x,y)
# p-value는 상관계수가 0이라고 했을때, 같은 크기의 표본에서
...
#1.49e-12=1.49 * 10의-12승

#Multiple R-squared:0.65
#모델 적합도, dist를 speed가 약 65% 설명한다
...
# -> 독립변수간 상관관계가 낮게(0.8미만) 나오도록 해야 함
# 
...
# 
# 
# #VIF(Variance Inflation Factor): 분산팽창요인 분석, 다중공선성 여부를 확인
# #일반적으로, VIF > 10 => 다중공선성 있음 => 변수제거(정보 손실이 적은 변수 제거)
# #경우에 따라, VIF > 3 또는 4 처럼 엄격하게 설정하기도 함
# 
# VIF = 1 / (1 - R^2)
...
#(모든 변수가 유의미하다면, VIF가 크게 나오더라도 모델 사용 가능)
# Coefficients:
...
#y=0.097113 * sat값 -0.046514 * weight값 + 0.053544 * width값 -0.936650


#회귀계수가 통계적으로 유의미하지 않다면,
#변수를 빼거나 새로운 변수를 생성
#ex) 국어점수 영어점수가 상관관계가 높다면 => 두 점수를 제거하고, 새로운 변수를
...
vif(model)
#weight,width 모두 기준값 10을 초과하지 않음 -> 회귀식 그냥 써도 됨

#연습삼아 변수 하나 제거하고  vif 조사해본다면,
summary(lm(y ~ sat + width, df))
vif(lm(y ~ sat + width, df))
vif(lm(y ~ sat + weight, df))
...
insu<-read.csv("insurance.csv", stringsAsFactors = TRUE)
str(insu)
summary(insu)
...
#insu에 age열 값에 대해 제곱한 값을 갖는 age2라는 열을 추가


insu$age2 <- insu$age **2

# library(dplyr)
# insu %>%  mutate(age2 = age**2)
# 
# insu$age2

head(insu[c('age','age2')])

#bmi 열 값이 30이상이면 1, 아니면 0 값을 갖는 bmi30 열을 추가하시오
